...
if ($w == "u")
{
    ...
    {
        $row = sql_fetch(" select bf_file, bf_content from g5_board_file where bo_table = '$bo_table' and wr_id = '$wr_id' and bf_no = '$i' ");
        if ($row[bf_file])
        {
            ...
            if ($is_file_content)
                ...
                $file_script .= "<br><input type='text' class=inputbox size=50 name='bf_content[$i]' value='".addslashes(get_text($row[bf_content]))."' title='업로드 이미지 파일에 해당 되는 내용을 입력하세요.'>";
            $file_script .= "\");\n";
        }
        else
...
$action_url = https_url(G5_BBS_DIR)."/write_update_iboard.php";
