...
<script>
	function go_per(){
		...
		location.href='<?=G5_URL?>/adm/alter_permission.php?mysql_db='+$("#mysql_db").val();
...
<section id="anc_cf_basic">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tbody>
				<tr>
            <th scope="row"><label for="cf_title">권한변경(FTP 아이디 입력)<strong class="sound_only">필수</strong></label></th>
            <td colspan="3">
							<input type="text" class="frm_input" name="mysql_db" id="mysql_db">
							<input type="button" class="btn_submit" value="게시판에 글쓴이 최고관리자 -> 관리자 로 모두 변경" onclick="go_per();" />
						</td>
        </tr>
        <tr>
...
<section id="anc_cf_extra">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <?php } ?>
            ...
            <td class="td_extra">
                소셜로그인&nbsp;&nbsp;
                <input type="radio" name="cf_9_subj" value="활성화"  class="cf_9_subj" style="width:15px;"<?if(eregi("활성화",$config[cf_9_subj])){?> checked <?}?> >활성화
								<input type="radio" name="cf_9_subj" value="비활성화" class="cf_9_subj" style="width:15px;"<?if(eregi("비활성화",$config[cf_9_subj])){?> checked <?}?>>비활성화&nbsp;&nbsp;&nbsp;&nbsp;
                소셜로그인범위&nbsp;&nbsp;&nbsp;&nbsp;
                <input type="checkbox" name="cf_9[]" value="facebook" style="width:15px;" <?if(eregi("facebook",$config[cf_9])){?> checked <?}?>>facebook
								<input type="checkbox" name="cf_9[]" value="naver" style="width:15px;" <?if(eregi("naver",$config[cf_9])){?> checked <?}?>>naver
								<input type="checkbox" name="cf_9[]" value="google" style="width:15px;" <?if(eregi("google",$config[cf_9])){?> checked <?}?>>google
								<input type="checkbox" name="cf_9[]" value="kakaotalk" style="width:15px;" <?if(eregi("kakaotalk",$config[cf_9])){?> checked <?}?>>kakaotalk
								<input type="checkbox" name="cf_9[]" value="instargram" style="width:15px;" <?if(eregi("instargram",$config[cf_9])){?> checked <?}?>>instargram
            </td>
        </tr>
				<tr>
            <th scope="row">여분필드10</th>
            <td class="td_extra">
                <label for="cf_10_subj">여분필드10 제목</label>
                <input type="text" name="cf_10_subj" value="" id="cf_10_subj" class="frm_input" size="30">
                <label for="cf_10">여분필드10 값</label>
                <input type="text" name="cf_10" value="<?php echo $config['cf_10'] ?>" id="cf_10" class="frm_input" size="30">
            </td>
        </tr>
