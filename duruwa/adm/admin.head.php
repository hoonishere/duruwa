...
{
    global $menu, $auth_menu, $is_admin, $auth, $g5, $config;
    $str .= "<ul class=\"gnb_2dul\">";
    for($i=1; $i<count($menu[$key]); $i++)
    {
        if ($is_admin != 'super' && (!array_key_exists($menu[$key][$i][0],$auth) || !strstr($auth[$menu[$key][$i][0]], 'r')) || (substr($menu[$key][$i][0],0,3) == "300" && $is_admin != "super"))
            continue;
		...
		if($is_admin != "super" && eregi("300",$config[cf_10]) && $key == "menu300") {
			$sql = " select * from g5_board order by bo_order desc ";
			$rowList = getList($sql);
			foreach($rowList as $line){
				$str .= "<li class='gnb_2dli'>";
				$str .= "<a class='gnb_2da' href=\"./board.php?bo_table={$line[bo_table]}\"";
				$str .= " class=\"strong\"";
				$str .=">{$span1}{$line[bo_subject]}</a></li>";
			}
		}
...
foreach($menu['menu'.$menu_key] as $key=>$value) {
		if($key > 0) {
				...
				if($menu_key != "300" || $is_admin == "super"){
					echo $nl.'<li><a href="'.$value[2].'">'.$value[1].'</a></li>';
				}

				if($is_admin != "super" && eregi("300",$config[cf_10]) && $menu_key == "300") {
					...
					foreach($rowList as $line){
						$nl2 .= "<li >";
						$nl2 .= "<a href=\"./board.php?bo_table={$line[bo_table]}\"";
						$nl2 .=">{$line[bo_subject]}</a></li>";
					}
					echo $nl2;
				}
...
<div id="wrapper">
    ...
    <div id="container">
        ...
        <h1 class="admin_tit"><?php echo $g5['title'] ?></h1>
