...
while ($entry = $tmp->read()) {
    if (!preg_match('/^admin.menu([0-9]{3}).*\.php$/', $entry, $m))
		...
		if($member[mb_level] < 10 && $member[mb_level] > 6 ){
			$arr_cf = @explode(",",$config[cf_10]);
			if(count($arr_cf) > 0){
				for($n = 0; count($arr_cf) > $n; $n++){
					$amenu[$arr_cf[$n]] = $entry;
					include_once(G5_ADMIN_PATH."/admin.menu".$arr_cf[$n].".php");
				}
			}
		}else{
			$amenu[$m[1]] = $entry;
