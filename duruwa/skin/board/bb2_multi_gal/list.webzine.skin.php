...
<ul class="board_webzine">
	<? for ($i=0; $i<count($list); $i++) { 
		...
		?>
	<li>
	<div class="dtlimg" style="position:absolute;top:10px;left:0px;z-index:100"><?=$image?></div>

		<div style="margin-left:270px;">
		<a href="<?=$list[$i][href]?>"><strong><?=$list[$i][subject]?></strong></a>
		<p class="wz_content" style="height:<?=($image_height+40)?>px;overflow:hidden;"><br><?=cut_str(strip_tags($list[$i][wr_content]),"500")?><br></p>
		</div>
