<?
$swfupload_path         = "$board_skin_url/swfupload"; // swfupload 경로
...
$file_allsize_limit     = "20 MB"; // 업로드 가능 총파일 용량
...
if ($wr_id) {
	$board_file_path    = G5_URL."/data/file/$bo_table"; // 파일폴더 경로
	$sql  = " select * from $g5[board_file_table] where bo_table = '$bo_table' and wr_id = '$wr_id' ";
} else {
	$board_file_path    = G5_URL."/data/guploader"; // 파일폴더 경로
	if(!is_dir($board_file_path)){mkdir($board_file_path);} //디렉토리가 없다면 생성.

    if (!sql_query("select count(*) as cnt from guploader",false)) { // guploader 테이블이 없다면 생성
        $sql_table = "create table guploader (
	...
	$sql  = " select * from guploader where bo_table = '$bo_table' and mb_id = '$member[mb_id]' and bf_ip = '$_SERVER[REMOTE_ADDR]' ";
}
...
// 업로드 상태확인
$uploader_status = "문서첨부제한 : " . getfilesize($sum_filesize) . " / " . $file_allsize_limit . "<br />파일제한크기 : " . getfilesize($board[bo_upload_size]) . " (허용확장자 : " . $file_types_description . ")";
...
var board_skin_path        = "<?=$board_skin_url?>"
...
<div id="swfup_content" style="padding:5px 0;">
    ...
    <col width=100 /><col width=5 /><col width=200 /><col width=5 /><col />
    	<tr>
            ...
            <td></td>
            <td><select name="uploaded_file_list" id="uploaded_file_list" style="width:200px; overflow:hidden;" size="5" onchange="preview();"><?=$file_list_option?></select></td>
            <td></td>
            <td valign="top">
				<div style="padding-bottom:10px;">
					<span id="spanButtonPlaceholder"></span>
					<span><input id="btnUpload" type="button" value="파일첨부" /></span>
					<span><input id="btnDelete" type="button" value="선택삭제" onClick="delete_file();" /></span>					
				</div>
				<div>
