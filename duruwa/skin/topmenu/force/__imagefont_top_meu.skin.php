...
/*.menu .major li.pactive ul.sub{display:block} 포커스시 서브메뉴도 보이게...*/
/*대메뉴 포커스 효과 변경시 */
...
$(function(){
	...
	function show_menu(){
		...
		$(".top_img").each(function(){ 		imgReplace($(this),"on"); });
		imgReplace(t.find("img"),"off");
	}
	// 서브메뉴 숨기기
	function hide_menu(){
		li_list.removeClass('active');
		...
		<?if($tp[m_menu] > 0 && !eregi("index.php",$_SERVER[SCRIPT_NAME])){?>
			...
			$(".top_img").each(function(){ 		imgReplace($(this),"on"); });
			imgReplace(li_list.eq(<?=($tp[m_menu]-1)?>).find("img"),"off");
			// 탑메뉴에 서브 포커스효과
	...
	<div id="menu" class="menu">
		<div class="major">
			...
			<?for($i = 0; $i < count($fmenu);$i++){?>
				<li class="m<?=($i+1)?>"><a href="<?=$fmenu[$i][1]?>"><span><img src="<?=$topmenu_skin_url?>/img/gnb_<?=($i+1)?>0_off.png" class="top_img"></span></a>
