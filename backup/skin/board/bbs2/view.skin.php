...
if($board[bo_2] == "youtube"){ // youtube일때
	include_once($board_skin_path."/view.youtube.skin.php");
	die();
}else if($board[bo_2] == "movie"){ // 일반 영상 넣을때...
	include_once($board_skin_path."/view.movie.skin.php");
	die();
}
?>
...
<ul class="write_info">
	...
	<?php if(number_format($view['wr_comment']) > 0){ echo "(".number_format($view['wr_comment']).")"; }?>
		 ...
		 <? if($bo_table == "52" && $member[mb_level] < 5){ ?>
		 <table class="guest_table">
        <tr>
            <th style="width:20%;"><label for="wr_1">이메일</label></th>
            <td><?php echo $write[wr_1] ?></td>
            <th style="width:20%;"><label for="wr_2">연락처</label></th>
            <td><?php echo $write[wr_2] ?></td>
        </tr>
		</table>
				<?}?>
	...
	<table class="guest_table">
		...
		<tr>
			...
			<td><?=$write[wr_1]?></td>
			<th>연락처</th>
			<td><?=$write[wr_2]?>-<?=$write[wr_3]?>-<?=$write[wr_4]?></td>
		</tr>
	</table>
	<?}?>
...
<!--     <div style="float:left;">
 ...
 -->
    ...
    <div style="float:right;">
		<? if ($is_print) { // 프린트 설정시?>
		<a href="#null" onclick="window.open('<?=$board_skin_url?>/print.skin.php?bo_table=<?=$bo_table?>&wr_id=<?=$wr_id?>','pop_print','width=1000,height=800,scrollbars=yes')">프린트</a>
		<? } ?>
