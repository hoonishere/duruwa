...
*/
  ...
  if ($padding < 0 || $padding > $mark_w || $padding > $mark_h) $padding = 0;//$padding이 0보다 작거나 워터마크의 너비나 높이보다 크면 10으로 강제 재설정
...
//thumnail(파일경로, 썸네일너비, 썸네일 높이, 퀄리티, 저장형태(0안만듬, 1: 없으면 만듬, 2: 새로만듬), 크롭사용,빌율에마춤, 노이미지, 워터마크경로)
function thumnail($path_src_file, $save_w, $save_h,$save_quality,$save_force=1,$crop_use,$pro_use="0", $noimage="",$watermark_path_file=""){
  ...
  $watermark_sharpness = 60;//워터마크 이미지의 투명도 : 30 %
  $watermark_padding = 0;//원본과 워터마크 사이의 여백
//$path_save_file
