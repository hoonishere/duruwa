...
if ($board['bo_use_category']) {
    ...
    if ($sca=='')
        $category_option .= ' id="bo_cate_on" class="color0"';
    ...
    for ($i=0; $i<count($categories); $i++) {
        ...
        if ($category==$sca) { // 현재 선택된 카테고리라면
            $category_option .= ' id="bo_cate_on"';
						if($bo_table=="52"||$bo_table=="31"){ $category_option .='class="color'.($i+1).'"' ; }
...
//echo "list_page_rows=> ".$list_page_rows."=page_rows=>".$page_rows."=page=>".$page ; 
...
/*
게시판 정렬 변경시 wr_2 desc
if ($bo_table == "41") {
    $sql_order = " order by wr_2 desc, {$sst} {$sod} ";
}
*/
